...
def SOCModel(X_train, Y_train, X_test, Y_test, X_eval, cv_iterator):
    svr_model, test_error_svr = SOC_SVR(X_train, Y_train, X_test, Y_test, cv_iterator)
    
    final_prediction = svr_model.predict(X_eval)
    
    return final_prediction, {'final':test_error_svr,'svr':test_error_svr}

def SOC_SVR(X_train, Y_train, X_test, Y_test, cv_iterator):
    ...
    # search.fit(X_train, Y_train["SOC"])
    # #search.grid_scores_
    #  
    ...
    test_error = math.sqrt(mean_squared_error(Y_test["SOC"], yhat_svr))
